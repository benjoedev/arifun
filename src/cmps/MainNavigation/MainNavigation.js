import React from 'react'

import { FaPlusCircle, FaRegLightbulb, FaUserCircle, FaSearch } from "react-icons/fa"
import './MainNavigation.css'
import {Link} from "react-router-dom";

const MainNavigation = () => {
    return (
        <div className='navigationContainer'>
            <div className='leftNavSide'>
                <img className='cardHeaderIcon'
                    src="data:image/png;base64,"
                    width={50}
                />


                {/*
                    Mock Link navigation
                */}
                <Link className='navLinkItem navLinkItem_clear-style' to='/dashboard'>Dashboard</Link>
                <Link className='navLinkItem navLinkItem_clear-style' to='/requests'>Requests</Link>
                <Link className='navLinkItem navLinkItem_clear-style' to='/activities'>Activities</Link>
                <Link className='navLinkItem navLinkItem_clear-style' to='/statics'>Statics</Link>
                <Link className='navLinkItem navLinkItem_clear-style' to='/leads'>Leads</Link>



            </div>
            <div className='rightNavSide'>
                <div className='searchWrapper'>
                    <input className='searchInput' type="text" />
                    <FaSearch className='searchIcon' size={20} />
                </div>
                <span className='navDivider'>|</span>
                <FaPlusCircle className='navIcon' size={20} />
                <span className='navDivider'>|</span>

                <FaRegLightbulb className='navIcon' size={20} />
                <span className='navDivider'>|</span>

                <FaUserCircle className='navIcon' size={20} />

                <span className='navUserName'>David Boranstein</span>

            </div>

        </div>
    )
}

export default MainNavigation